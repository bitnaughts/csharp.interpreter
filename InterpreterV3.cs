    ...
    public ClassObj(string input)
    {
        ...
        switch (input)
        {
            ...
            // "Booster l;", "Booster r;", "Thruster t;", "while (true) {", "l.Fire(Input.X);", "r.Fire(-Input.X);", "t.Fire(Input.Y);", "if (Input.UseWeapon()) {", "l.Fire(-1);", "r.Fire(-1);", "}", "}"
                ...
                //     new Field("Left = new Booster (1, 2, 3, 4);", "Booster")
                // });
                methods.Add(new Method(this, "Main", "_Entry_point_", "void", new List<Field>(){new Field("args", "String[]")}, new List<string>(){ "while (true)", "{", "$", "Engine.Throttle (System.in.Y * 25);", "Right.Boost (System.in.X * -50);", "if (Scanner.Scan () != 0 || System.in.Z)", "{", "Right.Launch ();", "Left.Launch ();", "}", "Left.Boost (System.in.X * 50);", "}" })); //"if (r.GetLength () > 0)", "{", "print (r.GetLength ());", "}", "}"})); //  "}", 
